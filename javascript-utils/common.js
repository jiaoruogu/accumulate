...
/**
 * 设定一个函数，使其在每天的一个固定时间执行一次
 * @returns null
 */

const timingExecute = (time = '00:00:00', callback) => {
  ...
  // 表示当前的时间点已经超过了该函数需要执行的时间，推迟至第二日执行
  if (delay < 0) {
  ...
  setTimeout(() => {
    callback();
    timingExecute(time, callback);
  }, target - new Date());
};

function runAtTime(targetHour, targetMinute, callback) {
  const now = new Date();
  const target = new Date(
    now.getFullYear(),
    now.getMonth(),
  ...
  const delay = target - now;
  if (delay < 0) {
    // 如果当前时间已经过了目标时间，则计算下一个目标时间并等待
    target.setDate(target.getDate() + 1);
  }
  setTimeout(() => {
    callback();
    runAtTime(targetHour, targetMinute, callback);
  }, target - new Date());
}

// 在每天的 10:30 执行函数
runAtTime(10, 30, () => {
  window.location = "要定时刷新页面的地址";
});
